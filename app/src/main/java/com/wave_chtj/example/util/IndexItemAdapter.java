...
public class IndexItemAdapter extends BaseMultiItemQuickAdapter<Dbean, BaseViewHolder> {
    public static final int L_NO_BG = 1;
    public static final int L_ONE = 2;

    //构造方法，传入数据
    public IndexItemAdapter(List<Dbean> itemList) {
        super(itemList);
    ...
    protected void convert(BaseViewHolder helper, Dbean dbean) {
        switch (helper.getItemViewType()) {
            case IndexItemAdapter.L_NO_BG:
                break;
            case IndexItemAdapter.L_ONE:
                break;
        }
